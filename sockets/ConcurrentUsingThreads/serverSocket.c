...
doServiceFork(int fd) {
	pthread_t thread;
	int ret = pthread_create(&thread, NULL, doService, (void *)fd);
	if (ret < 0) perror("Error en la creacion del thread");
	
	//no hace falta liberar los threads, lo hace automaticamente el so
}
