...
int createServerSocket (int port)
{
	...
	direccion.sin_port = htons(port); //Para no interferir con sistema > 5000, se pasa en el server
	...
	err = listen(fd,10);
	if (err < 0) {
		perror ("Error en configurar conexiones; funcion listen()");
		exit(1);
	}
	return fd;
}
...
int acceptNewConnections (int socket_fd)
{
	...
	int size = sizeof(direccion);
	int fd = accept(socket_fd, &direccion, &size);  //en este caso es un puntero el size! No es como en bind()
	if (fd < 0) {
		perror ("Error al esperar la solicitud de conexion; funcion accept()");
		exit(1);
		return fd;
	}
	return fd;
